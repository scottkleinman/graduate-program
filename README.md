...
The injector now computes `git_revision_date_localized` using the latest commit that made a meaningful change to each page file, ignoring commits that only update managed date keys in front matter. This prevents publish-time date churn from bumping "Last updated" across the site.
